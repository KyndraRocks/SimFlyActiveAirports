...
**Cross-app pilot group launch** — passing a `pilots=` URL parameter (comma-separated pilot names, e.g. `?pilots=Alice,Bob`) opens the app directly into Region Mode with the Airport Owners filter pre-set to exactly those pilots, and the map auto-switched to **owner colors** so each pilot's airports are immediately distinguishable. (You can still toggle to category or payout colors via the 🎨 button afterward.) The SimFly Reciprocation Tracker and Relationship Manager Lite both use this to send a selected group from the Balance Queue straight to the region map with one click.

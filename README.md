...
**One-click clearing** (v2.84.1+) — the bottom of the **🔌 Live** pulldown has two destructive-action rows below a divider:
- **✕ Clear Flight Plan** removes the plotted SimBrief route from the map and wipes the local-storage cache so it won't restore on next page load. Waypoint-alarm rings clear alongside.
- **✕ Clear Aircraft Track** erases the green breadcrumb trail (all three tiers — ultra-fresh, fresh, archive) and wipes the local-storage track. Both rows toast a confirmation on success and a soft *"nothing to clear"* warning when nothing's loaded.
...
- **⊙ Geofence** — picking this opens a small chooser first: **Departure airport**, **Arrival airport**, or **Draw on map**. The first two are *plan-relative* — the fence auto-relocates to wherever DEP/ARR is in the currently-loaded SimBrief plan, so a single fence works across every route without re-drawing. Pick **Draw on map** to click and drag a circular region of any fixed lat/lng. Choose **ENTER**, **EXIT**, or **both** as the trigger. Plan-relative fences re-anchor automatically whenever a new SimBrief plan loads, and their map label shows `@DEP KJFK` / `@ARR EGLL` so they're obvious at a glance. Switch any existing fence between Coords / DEP / ARR via the **Anchor** dropdown on its alarm card; the **Radius** input is always editable. Orange ring persists on the map; pulses red when tripped.
